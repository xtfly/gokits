...
func TestTaskGroupNormal(t *testing.T) {
	var (
...
func TestTaskGroupGOMAXPROCS(t *testing.T) {
	// 没有并发数限制
	g := TaskGroup{}
	now := time.Now()
	g.Go(sleep1s)
	...
	// context canceled
	var canceled bool
	g3 := NewTaskGroupWithCancel(context.Background())
	g3.GOMAXPROCS(2)
...
func TestTaskGroupRecover(t *testing.T) {
	var (
		abcs = make(map[int]*ABC)
		g    TaskGroup
		err  error
...
func TestTaskGroupRecover2(t *testing.T) {
	var (
		g   TaskGroup
		err error
	)
...
func ExampleTaskGroupJustErrors() {
	var g TaskGroup
	var urls = []string{
		"http://www.golang.org/",
		"http://www.google.com/",
...
// and error-handling.
func ExampleGroupParallel() {
	Google := func(ctx context.Context, query string) ([]Result, error) {
		g := NewTaskGroupWithContext(ctx)
...
func TestNewTaskGroupWithCancel(t *testing.T) {
	g := NewTaskGroupWithCancel(context.Background())
